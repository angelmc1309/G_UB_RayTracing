...
BoundaryObject::BoundaryObject(string s, float data) : Object(data)
{
  ...
  for (Cara c : cares) {
      triangles.push_back(Triangle(
              (glm::vec3) vertexs[c.idxVertices[0]],
              (glm::vec3) vertexs[c.idxVertices[1]],
              (glm::vec3) vertexs[c.idxVertices[2]],
              -1));
  }
...
bool BoundaryObject::hit(const Ray& raig, float t_min, float t_max, HitInfo& info) const {
    bool hit = false;
    //TODO remove from hit, it does affect performance.
    HitInfo *nearestInfo = new HitInfo;

    nearestInfo-> t = 1.0e+9;

    for (Triangle tr : triangles){
        if (tr.hit(raig, t_min, t_max, info) ) {
            hit = true;
            if (info.t < nearestInfo->t) {
                nearestInfo->t = info.t;
                nearestInfo->normal = info.normal;
                nearestInfo->p = info.p;
                nearestInfo->mat_ptr = material.get();
            }
    ...
    if (hit) {
        info.t = nearestInfo->t;
        info.normal = nearestInfo->normal;
        info.p = nearestInfo->p;
...
BoundaryObject::BoundaryObject(const QString &fileName, float data): Object(data)
{
    ...
    if(file.exists()) {
        ...
        for (Cara c : cares) {
              ...
              triangles.push_back(Triangle(
                      (glm::vec3) vertexs[c.idxVertices.at(0)],
                      (glm::vec3) vertexs[c.idxVertices.at(1)],
                      (glm::vec3) vertexs[c.idxVertices.at(2)],
                      -1));
